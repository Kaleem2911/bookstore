...
const Cart = () => {
  ...
  useEffect(() => {
    axios.get('https://bookstore-1-yy82.onrender.com/get-cart-books', { headers })
      .then((res) => {
  ...
  const handleRemove = async (bookid) => {
    try {
      const response = await axios.delete(
        'https://bookstore-1-yy82.onrender.com/removeBookFromCart',
        {
          headers: {
            ...headers,
            bookid,
  ...
  const handleOrderHistory = async () => {
    try {
      ...
      const response = await axios.post(
        'https://bookstore-1-yy82.onrender.com/place-order',
        { order: cartBooks },
        { headers }
      );
