...
const BookDetail = () => {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      try {
        const { data } = await axios.get(`https://bookstore-zqy9.onrender.com/get-book/${id}`);
  ...
  const handleFavourites = async () => {
    try {
      const response = await axios.put('https://bookstore-zqy9.onrender.com/add-book-to-favourites', {}, { headers });
  ...
  const handleCart = async () => {
    try {
      const response = await axios.put('https://bookstore-zqy9.onrender.com/addToCart', {}, { headers });
      if (response.status === 200) {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`https://bookstore-zqy9.onrender.com/delete-books/${id}`, { headers });
