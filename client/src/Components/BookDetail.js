...
const BookDetail = () => {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      try {
        const { data } = await axios.get(`https://localhost:5000/get-book/${id}`);
  ...
  const handleFavourites = async () => {
    try {
      const response = await axios.put('https://localhost:5000/add-book-to-favourites', {}, { headers });
  ...
  const handleCart = async () => {
    try {
      const response = await axios.put('https://localhost:5000/addToCart', {}, { headers });
      if (response.status === 200) {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`https://localhost:5000/delete-books/${id}`, { headers });
