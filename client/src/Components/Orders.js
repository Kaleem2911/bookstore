...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get("https://bookstore-1-yy82.onrender.com/get-all-orders", { headers });
  ...
  const handleDelete = async (orderId) => {
    try {
      await axios.delete(`https://bookstore-1-yy82.onrender.com/delete-order/${orderId}`, { headers });
